...
void applyEffectGlobally(Iteration* iteration, Cell* cell, int pos[2], int min_index[2], int max_index[2]);
...
void sendCellsConditions(Iteration* iteration);
void applyCellMessageEffects(Iteration* iteration, CellMessage* message, int count, int* block);
void queueCellForBlock(Iteration* iteration, int position[2], int* block);
...
void spreadCellCondition(Iteration* iteration, int position[2]){
	...
	applyEffectGlobally(iteration, cell, position, min_index, max_index);
}
	...
	for (int block_row = min_block[1]; block_row <= max_block[1]; block_row++){
		...
		for (int block_col = min_block[0]; block_col <= max_block[0]; block_col++){
			...
			if (isBlockLocal(to_block, parameters)){
				...
				queueCellForBlock(iteration, pos, to_block);
			}
		}
	}
}
...
void queueCellForBlock(Iteration* iteration, int position[2], int* block){
	...
	CellPosition* loc = getCellPosition(map, col, row);
	loc->x = position[0];
	loc->y = position[1];
}
...
void sendCellsConditions(Iteration* iteration){
	...
	for (int row = 0; row < map_dims[1]; row++){
		...
		for (int col = 0; col < map_dims[0]; col++){
			...
			if (send_count > 0){
				sendBlockFromPositions(send_cells, send_count, map, block, parameters);
...
void receiveAnyCellConditions(Iteration* iteration){
	...
	while (hasMessages()){
		receiveCellArrayAtBlock(message, &count, block_pos, parameters);
